...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    res.cookie("email", checkuser.email);
    res.cookie("mobile", checkuser.mobile).redirect("/getstarted");
  } catch (error) {
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!emailormobile || !password) {
      return res
        .status(400)
        .json({ msg: "Email/Mobile and Password are required" });
    }
    ...
    let token = jwt.sign(userResponse, process.env.secretkey, {
      expiresIn: "2h",
    });
    ...
    return res.status(201)
        ...
        .cookie("name",userResponse.name)
        .cookie("email",userResponse.email)
        .cookie("mobile",userResponse.mobile)
        .redirect("/getstarted");
